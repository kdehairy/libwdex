 ...
 * only; it does not verify that access flags, indices, or offsets
 * are valid. */
DexClassData* dexReadAndVerifyClassData( u1** pData, const u1* pLimit);
 ...
 * data. */
DEX_INLINE void dexReadClassDataHeader( u1** pData,
        DexClassDataHeader *pHeader) {
    pHeader->staticFieldsSize = readUnsignedLeb128((const u1**)pData);
    pHeader->instanceFieldsSize = readUnsignedLeb128((const u1 **)pData);
    pHeader->directMethodsSize = readUnsignedLeb128((const u1 **)pData);
    pHeader->virtualMethodsSize = readUnsignedLeb128((const u1 **)pData);
}
 ...
 * decode process.
 */
DEX_INLINE void dexReadClassDataField( u1** pData, DexField* pField,
        u4* lastIndex) {
    u4 index = *lastIndex + readUnsignedLeb128((const u1 **)pData);

    pField->accessFlags = readUnsignedLeb128((const u1 **)pData);
 ...
 * decode process.
 */
DEX_INLINE void dexReadClassDataMethod( u1** pData, DexMethod* pMethod,
        u4* lastIndex) {
    u4 index = *lastIndex + readUnsignedLeb128((const u1 **)pData);

    pMethod->accessFlags = readUnsignedLeb128((const u1 **)pData);
    pMethod->codeOff = readUnsignedLeb128((const u1 **)pData);
